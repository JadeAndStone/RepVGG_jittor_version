...
device='cuda:0'
batch_size=100
train_set,val_set=pytorch_Img_Dataset.build_dataset()
    ...
    def fuse_bn_Var(self,branch):
        ...
        if isinstance(branch,nn.Sequential):
            if branch.conv.kernel_size==(1,1):
                kernel=F.pad(branch.conv.weight,[1,1,1,1])
            else:
            ...
            pass
            kernel=torch.zeros((self.rbr_dense.conv.out_channels,self.rbr_dense.conv.in_channels,3,3)).to(device)
            ...
            beta=branch.bias
            eps=branch.eps
        return kernel*(gamma/(torch.sqrt(running_var+eps))).reshape((-1,1,1,1)),beta-running_mean*gamma/(torch.sqrt(running_var+eps))
...
net=RepVGG(conv_arch).to(device)
...
#     y_hat=np.array(y_hat.to('cpu')).argsort(axis=1)
#     y=y.to('cpu')
#     c=0
...
def top5_accuracy(y_hat, y):
    top5=torch.topk(y_hat,k=5,dim=1).indices
    match=top5.eq(y.view(-1, 1))
...
trainer=torch.optim.AdamW(net.parameters(),lr=1e-3,weight_decay=0.05)
num_epoch=50
train_loss_total=[]
val_loss_total=[]
...
# plt.legend()
# plt.grid(True)
# plt.savefig('pytorch_RepVGG Loss Curve.png')
# plt.show()
