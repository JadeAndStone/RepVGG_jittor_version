...
def build_dataset():
    ...
    train_img_form=transform.Compose([
        transform.Resize(128),
        transform.RandomResizedCrop(96,scale=(0.8,1.0)),
        # transform.Resize(72),
    ...
    val_img_form=transform.Compose([
        transform.Resize(128),
        transform.CenterCrop(96),
        transform.ToTensor(),
        transform.ImageNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
    ...
    train_set.set_attrs(shuffle=True,num_workers=4)
    val_set.set_attrs(shuffle=True,num_workers=4)
